...
| 3 | `wipeComposition()` | GET composition → loop POST `/clips/{m}/clear` for every slot | `POST /composition/clear`? | SPECULATIVE | If exists, replaces O(layers × clips) POSTs with one. Alternative: parallelize current loop with `Promise.all` + 4-way semaphore. |
...
| 8 | `setBpm(b)` | PUT `/composition` with `{tempocontroller:{tempo:{value:b}}}` | `PUT /composition/tempocontroller/tempo` with `{value:b}` | SPECULATIVE | If deep path exists, body shrinks. Watch unit (REST=raw BPM, OSC=normalized). |
| 9 | `tapTempo()` | PUT with `{tempocontroller:{tempo_tap:{value:true}}}` | `POST /composition/tempocontroller/tempo_tap`? | SPECULATIVE | "Tap" is fundamentally an action. Probe. |
| 10 | `resyncTempo()` | PUT with `{tempocontroller:{resync:{value:true}}}` | `POST /composition/tempocontroller/resync` | SPECULATIVE — **likely exists** | CLAUDE.md lists `tempocontroller/resync` as known OSC trigger; OSC trigger paths usually map 1:1 to REST POST. **High priority.** |
| 11 | `triggerClip(n,m)` | `POST /composition/layers/{n}/clips/{m}/connect` | **Already optimal** | CONFIRMED | Same path Bitfocus Companion uses. |
| 12 | `setCrossfader(p)` | PUT `/composition` with `{crossfader:{phase:{value:p}}}` | `PUT /composition/crossfader/phase` with `{value:p}` | SPECULATIVE | Same family as #8. |
...
- **Confirmed negative** (don't pursue): #4, #5, #14.
- **Speculative — probe first**: #3, #8, #9, #10, #12. **#10 highest priority.**
- **Derived options-caching opportunities** (v0.6): #6, #7, #13.

**Net v0.5 work**: probe #3/#8/#9/#10/#12 against live Resolume Swagger; convert any that exist; document non-existent ones in CLAUDE.md.
