...
describe("EffectIdCache requireRevalidation flag (v0.5.2 silent-no-op fix)", () => {
  it("skips the cache write on the very first MISS-fetch after invalidate(layer, {requireRevalidation:true})", async () => {
    // Live-discovered: post-add Resolume returns a transient effect id.
    // The first PUT lands; the second silently no-ops. To dodge that we
    // must not cache the first MISS — the next call refetches against a
    // now-stable id.
    const cache = new EffectIdCache();
    let next = 999; // transient id on first fetch
    const fetcher = vi.fn(async () => next);

    // Populate normally first.
    await cache.lookup(2, 3, fetcher);
    expect(cache.size).toBe(1);

    // Invalidate with the revalidation flag.
    cache.invalidateLayer(2, { requireRevalidation: true });
    expect(cache.size).toBe(0);

    // First MISS post-invalidate: fetches but does NOT cache.
    next = 999;
    expect(await cache.lookup(2, 3, fetcher)).toBe(999);
    expect(cache.size).toBe(0);

    // Second MISS: fetches with the stable id and caches it.
    next = 42;
    expect(await cache.lookup(2, 3, fetcher)).toBe(42);
    expect(cache.size).toBe(1);

    // Third call: HIT — no fetcher call, returns the stable id.
    next = -1;
    expect(await cache.lookup(2, 3, fetcher)).toBe(42);
    expect(fetcher).toHaveBeenCalledTimes(3);
  });

  it("default invalidateLayer (no options) caches the first MISS-fetch normally", async () => {
    // Backward compatibility — non-add invalidations (e.g. clearAll, remove)
    // shouldn't waste an extra GET on phantom verification.
    const cache = new EffectIdCache();
    const fetcher = vi.fn(async () => 7);
    await cache.lookup(1, 1, fetcher);
    cache.invalidateLayer(1); // no opts
    expect(await cache.lookup(1, 1, fetcher)).toBe(7);
    expect(cache.size).toBe(1);
  });

  it("requireRevalidation flag is per-layer", async () => {
    // Marking layer 2 as stabilizing must not affect layer 3's caching.
    const cache = new EffectIdCache();
    cache.invalidateLayer(2, { requireRevalidation: true });
    const f3 = vi.fn(async () => 33);
    await cache.lookup(3, 1, f3);
    // Layer 3 cached normally on first MISS.
    expect(cache.size).toBe(1);
    expect(await cache.lookup(3, 1, f3)).toBe(33);
    expect(f3).toHaveBeenCalledTimes(1);
  });

  it("clearAll resets the stabilization marker", async () => {
    const cache = new EffectIdCache();
    cache.invalidateLayer(2, { requireRevalidation: true });
    cache.clearAll();
    // After clearAll, layer 2 should cache normally on first MISS.
    const fetcher = vi.fn(async () => 99);
    await cache.lookup(2, 1, fetcher);
    expect(cache.size).toBe(1);
  });

  it("a non-stabilizing invalidate clears a previously-set stabilization marker", async () => {
    // Edge case: addEffectToLayer marks layer 2 stabilizing → user calls
    // a different code path that does plain invalidateLayer(2). The plain
    // path should clear the marker so we resume normal caching.
    const cache = new EffectIdCache();
    cache.invalidateLayer(2, { requireRevalidation: true });
    cache.invalidateLayer(2); // plain — clears marker
    const fetcher = vi.fn(async () => 5);
    await cache.lookup(2, 1, fetcher);
    expect(cache.size).toBe(1); // cached on first MISS
  });
});
